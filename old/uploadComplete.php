...
while ($row = mysql_fetch_assoc($result)) {
	...
	$outlier[$row['outlierID']]['outlierLevel'] = $row['outlierLevel'];
...
while (!feof($file_handle)) {
 ...
 if (($startFlag == "Y")  && !(strpos($line,"\t") == "0")) {
	...
	if (!$platformID && $platform){
		...
		$screenOutput .= "<br /><b>New Platform set up: " . $platform . "   <a href='editPublisherPlatform.php?platformID=" . $platformID . "'>edit</a></b>";
	...
	#################################################################
	//Clean some of the data
	//replace single quote with '' so SQL doesn't break
	...
	if (!$titleID && $journalTitle && ((strlen($printISSN) == "8") || !$printISSN)){
		...
		if (strlen($printISSN) == "8") {
			mysql_query("insert into title_issn (titleID, ISSN, ISSNType) values ('$titleID', '$printISSN', 'print');");
		}

		if (strlen($onlineISSN) == "8") {
			mysql_query("insert into title_issn (titleID, ISSN, ISSNType) values ('$titleID', '$onlineISSN', 'online');");
		}
...
$query = "insert into import_log (netID, fileName, archiveFileURL, logFileURL, details) values ('$netID', '$orgFileName', '" . $uploadedFile . "', '" . $logfile . "', '" . $rownumber . " titles processed.');";
mysql_query($query);
