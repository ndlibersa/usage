	...
	if (count($errorMessage) > 0){
		...
		if (!$link) {
			...
			if (!$dbcheck) {
				...
				$query = "SELECT count(*) count FROM information_schema.`COLUMNS` WHERE table_schema = '" . $database_name . "' AND table_name='Publisher'";

				//if publisher table exists, error out
				if (!$row = mysqli_fetch_array(mysqli_query($link, $query))){
					...
					if ($row['count'] > 0){
						$errorMessage[] = "The Usage tables already exist.  If you intend to upgrade, please run upgrade.php instead.  If you would like to perform a fresh install you will need to manually drop all of the Usage tables in this schema first.";
					}else{
...
<?php if(!$step){ ?>
	...
	<ul>
		...
		<li>Connect to MySQL and create the CORAL Usage tables</li>
		<li>Test the database connection the CORAL Usage application will use </li>
	...
	<ul>
		<li>Create a MySQL Schema created for CORAL Usage Module - recommended name is coral_usage_prod.  Each CORAL module has separate user permissions and requires a separate schema.</li>
...
//fourth step - ask for other settings in configuration.ini
	...
	if ($_POST['reportingModule']) $reportingChecked = "checked";
...
<?php
//fifth step - ask for other settings in configuration.ini
}else if ($step == '5'){ ?>
	...
	<ul>
		...
		<li>Change permissions from the /admin/ directory to restrict read/write access to user only
		<li>Verify the following directories are web server writable:  logs, archive and sushistore
		<li>If you will be using SUSHI, make sure your php installation has the soap client installed (to verify, visit your php.ini file or view phpinfo)
		<ul><li>For more info on php info:  http://php.net/manual/en/function.phpinfo.php</li>
			<li>For more info on soap client: http://php.net/manual/en/class.soapclient.php</li>
		</ul>
		</li>

		<li>If you wish to use the sushi scheduler to automatically run sushi imports you will need a scheduler (cron) to run:
		sushiSchedule.php</li>
