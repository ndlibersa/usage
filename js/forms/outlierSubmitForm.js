...
function validateForm (monthlyUsageSummaryIDValue){
	myReturn=0;
	if (!validateRequired('overrideUsageCount_' + monthlyUsageSummaryIDValue, '<br />Count is required.  If you wish to remove the count the outlier should be ignored.')) myReturn="1";
	if (myReturn == "1"){
		return false;
	}else{
		return true;
