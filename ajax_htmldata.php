...
switch ($action) {
    ...
    case 'getLoginDetails':
		...
		if ($config->settings->organizationsModule == 'Y'){
			echo "<br /><br /><br /><h3>"._("Organization Accounts")."</h3>";
    ...
    case 'getOutlierTable':
		...
		if ($config->settings->useOutliers == "Y"){
			...
			foreach($outliers->allAsArray as $outlier) {
				echo "Level " . $outlier['outlierLevel'] . ": " . $outlier['overageCount'] . _(" over plus ") .  $outlier['overagePercent'] . '% ' ._("over - displayed ") . $outlier['color'];
		...
		if (count($user->allAsArray()) > 0){
			...
			<table class='dataTable' style='width:550px'>
				...
				foreach($user->allAsArray() as $instance) {
					...
					echo "<td style='width:70px'><a href='ajax_forms.php?action=getAdminUserUpdateForm&loginID=" . $instance['loginID'] . "&height=196&width=248&modal=true' class='thickbox' id='expression'>"._("update")."</a></td>";
